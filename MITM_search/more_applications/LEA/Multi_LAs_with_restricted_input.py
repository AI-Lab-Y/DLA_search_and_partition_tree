'''
The SAT-based model for searching linear approximations
'''
...
bit_bias = np.load('./8r_oneBitDiff_oneBitMask_cor_w.npy')
bit_bias_bound = 8
diff_index = 127
active_input_bits = [i for i in range(128) if bit_bias[diff_index][i] < bit_bias_bound]
...
def CountClausesInRoundFunction(Round, Bias, clausenum):
    ...
    for r in range(Round):
        # three modular operations
        for j in range(3):
            count += 1
            count += 8
            for i in range(QuarterBlockSize - 2):
                count += 16
...
def gen_Constraints_For_Round_function(xin, xout, w, a, b, c, d, e, Round, fout):
    for round in range(Round):
        # for the first modular
        # the first condition
        ...
        x = a[round][0] + 1
        y = b[round][0] + 1
        z = xout[round][(QuarterBlockSize - alpha) % QuarterBlockSize] + 1
        w1 = w[round][1] + 1
        ...
        for i in range(QuarterBlockSize - 2):
            x = a[round][i + 1] + 1
            y = b[round][i + 1] + 1
            z = xout[round][(i + 1 + QuarterBlockSize - alpha) % QuarterBlockSize] + 1
            w1 = w[round][i + 1] + 1
            w2 = w[round][i + 2] + 1
        ...
        for i in range(QuarterBlockSize):
            x = a[round][i] + 1
            y = b[round][i] + 1
            z = xout[round][(i + QuarterBlockSize - alpha) % QuarterBlockSize] + 1
            w1 = w[round][i] + 1
            clauseseq = str(x) + " " + "-" + str(z) + " " + str(w1) + " 0\n"
            fout.write(clauseseq)
            clauseseq = "-" + str(x) + " " + str(z) + " " + str(w1) + " 0\n"
            fout.write(clauseseq)
            clauseseq = str(y) + " " + "-" + str(z) + " " + str(w1) + " 0\n"
            fout.write(clauseseq)
            clauseseq = "-" + str(y) + " " + str(z) + " " + str(w1) + " 0\n"
            fout.write(clauseseq)

        # for the second modular
        # the first condition
        clauseseq = "-" + str(w[round][0 + QuarterBlockSize] + 1) + " 0\n"
        fout.write(clauseseq)
        # the second condition
        x = c[round][0] + 1
        y = d[round][0] + 1
        z = xout[round][(0 + beta) % QuarterBlockSize + QuarterBlockSize] + 1
        w1 = w[round][1 + QuarterBlockSize] + 1
        clauseseq = str(x) + " " + str(y) + " " + str(z) + " " + "-" + str(w1) + " 0\n"
        fout.write(clauseseq)
        clauseseq = str(x) + " " + str(y) + " " + "-" + str(z) + " " + str(w1) + " 0\n"
        fout.write(clauseseq)
        ...
        for i in range(QuarterBlockSize - 2):
            x = c[round][i + 1] + 1
            y = d[round][i + 1] + 1
            z = xout[round][(i + 1 + beta) % QuarterBlockSize + QuarterBlockSize] + 1
            w1 = w[round][i + 1 + QuarterBlockSize] + 1
            w2 = w[round][i + 2 + QuarterBlockSize] + 1
            clauseseq = str(x) + " " + str(y) + " " + str(z) + " " + str(w1) + " " + "-" + str(w2) + " 0\n"
            fout.write(clauseseq)
            clauseseq = str(x) + " " + str(y) + " " + str(z) + " " + "-" + str(w1) + " " + str(w2) + " 0\n"
            fout.write(clauseseq)
        ...
        for i in range(QuarterBlockSize):
            x = c[round][i] + 1
            y = d[round][i] + 1
            z = xout[round][(i + beta) % QuarterBlockSize + QuarterBlockSize] + 1
            w1 = w[round][i + QuarterBlockSize] + 1
            clauseseq = str(x) + " " + "-" + str(z) + " " + str(w1) + " 0\n"
            fout.write(clauseseq)
            clauseseq = "-" + str(x) + " " + str(z) + " " + str(w1) + " 0\n"
            fout.write(clauseseq)
        ...
        # for the third modular
        # the first condition
        clauseseq = "-" + str(w[round][0 + QuarterBlockSize * 2] + 1) + " 0\n"
        fout.write(clauseseq)
        # the second condition
        x = e[round][0] + 1
        y = xin[round][0 + QuarterBlockSize * 3] + 1
        z = xout[round][(0 + gamma) % QuarterBlockSize + QuarterBlockSize * 2] + 1
        w1 = w[round][1 + QuarterBlockSize * 2] + 1
        clauseseq = str(x) + " " + str(y) + " " + str(z) + " " + "-" + str(w1) + " 0\n"
        fout.write(clauseseq)
        clauseseq = str(x) + " " + str(y) + " " + "-" + str(z) + " " + str(w1) + " 0\n"
        fout.write(clauseseq)
        ...
        for i in range(QuarterBlockSize - 2):
            x = e[round][i + 1] + 1
            y = xin[round][i + 1 + QuarterBlockSize * 3] + 1
            z = xout[round][(i + 1 + gamma) % QuarterBlockSize + QuarterBlockSize * 2] + 1
            w1 = w[round][i + 1 + QuarterBlockSize * 2] + 1
            w2 = w[round][i + 2 + QuarterBlockSize * 2] + 1
            clauseseq = str(x) + " " + str(y) + " " + str(z) + " " + str(w1) + " " + "-" + str(w2) + " 0\n"
            fout.write(clauseseq)
            clauseseq = str(x) + " " + str(y) + " " + str(z) + " " + "-" + str(w1) + " " + str(w2) + " 0\n"
            fout.write(clauseseq)
        ...
        for i in range(QuarterBlockSize):
            x = e[round][i] + 1
            y = xin[round][i + QuarterBlockSize * 3] + 1
            z = xout[round][(i + gamma) % QuarterBlockSize + QuarterBlockSize * 2] + 1
            w1 = w[round][i + QuarterBlockSize * 2] + 1
            clauseseq = str(x) + " " + "-" + str(z) + " " + str(w1) + " 0\n"
            fout.write(clauseseq)
            clauseseq = "-" + str(x) + " " + str(z) + " " + str(w1) + " 0\n"
            fout.write(clauseseq)
        ...
        for i in range(QuarterBlockSize):
            x = xin[round][i] + 1
            y = xout[round][i + QuarterBlockSize * 3] + 1
            z = a[round][i] + 1
        ...
        # the second branch
        for i in range(QuarterBlockSize):
            x = xin[round][i + QuarterBlockSize] + 1
            y = b[round][i] + 1
            z = c[round][i] + 1
            clauseseq = str(x) + " " + str(y) + " " + "-" + str(z) + " 0\n"
            fout.write(clauseseq)
        ...
        # the third branch
        for i in range(QuarterBlockSize):
            x = xin[round][i + QuarterBlockSize * 2] + 1
            y = d[round][i] + 1
            z = e[round][i] + 1
            clauseseq = str(x) + " " + str(y) + " " + "-" + str(z) + " 0\n"
            fout.write(clauseseq)
...
def gen_constraint_for_fixed_state(sta, val, fout):
    # add constraints to claim fixed input mask
    m0, m1, m2, m3 = val[0], val[1], val[2], val[3]
    for i in range(QuarterBlockSize):
        if (m0 >> (QuarterBlockSize - 1 - i)) & 1 == 1:
            clauseseq = str(sta[i] + 1) + " 0\n"
            fout.write(clauseseq)
        else:
            clauseseq = "-" + str(sta[i] + 1) + " 0\n"
            fout.write(clauseseq)
    for i in range(QuarterBlockSize):
        if (m1 >> (QuarterBlockSize - 1 - i)) & 1 == 1:
            clauseseq = str(sta[i + QuarterBlockSize] + 1) + " 0\n"
            fout.write(clauseseq)
        else:
            clauseseq = "-" + str(sta[i + QuarterBlockSize] + 1) + " 0\n"
            fout.write(clauseseq)
    for i in range(QuarterBlockSize):
        if (m2 >> (QuarterBlockSize - 1 - i)) & 1 == 1:
            clauseseq = str(sta[i + QuarterBlockSize * 2] + 1) + " 0\n"
            fout.write(clauseseq)
        else:
            clauseseq = "-" + str(sta[i + QuarterBlockSize * 2] + 1) + " 0\n"
            fout.write(clauseseq)
    for i in range(QuarterBlockSize):
        if (m3 >> (QuarterBlockSize - 1 - i)) & 1 == 1:
            clauseseq = str(sta[i + QuarterBlockSize * 3] + 1) + " 0\n"
            fout.write(clauseseq)
        else:
            clauseseq = "-" + str(sta[i + QuarterBlockSize * 3] + 1) + " 0\n"
            fout.write(clauseseq)
...
# add conditions for linear masks that have been searched
...
def Decision(Round, Bias, MatsuiRoundIndex, MatsuiCount, flag, num=0, folder=None):
    TotalBias = QuarterBlockSize * 3 * Round
    count_var_num = 0
    ...
    for i in range(Round):
        ...
        for j in range(BlockSize):
            xin[i].append(0)
        for j in range(QuarterBlockSize):
            a[i].append(0)
            b[i].append(0)
            c[i].append(0)
            d[i].append(0)
            e[i].append(0)
        for j in range(QuarterBlockSize * 3):
            w[i].append(0)
        for j in range(BlockSize):
            xout[i].append(0)
    # Allocate variable
    ...
    Main_Var_Num = QuarterBlockSize * 3 * Round
    ...
    #     LeftNode = QuarterBlockSize * 3 * StartingRound
    #     RightNode = QuarterBlockSize * 3 * EndingRound - 1
    ...
    # count the number of clauses for fixed state
    ...
    # Add constraints to claim nonzero input mask
    clauseseq = ""
    ...
    # Round function
    gen_Constraints_For_Round_function(xin=xin, xout=xout, w=w, a=a, b=b, c=c, d=d, e=e,
                                       Round=Round, fout=file)
    # Add constraints for the original sequential encoding
    Main_Vars = list([])
    for r in range(Round):
        for i in range(QuarterBlockSize * 3):
    ...
    # add constraints for linears masks/hulls that have been searched
...
# main function
folder = './linear_hulls/cor_{}/diff_{}/{}_round/'.format(UpperBound, diff_index, SearchRoundStart)
